...
export default function Home() {
      ...
      <main className="h-screen flex items-center justify-center bg-gray-100">
        <div className="bg-white p-8 rounded-lg shadow-md w-80 text-center">
          ...
          <button
            ...
            className="w-full bg-[#1e293b] text-white rounded py-2 text-sm hover:bg-[#0f172a] flex items-center justify-center gap-2"
          >
  ...
  return (
    <main className="h-screen flex flex-col overflow-hidden">
      {/* Header - compact */}
      <div className="flex items-center justify-between px-4 py-2 bg-[#1e293b] shadow-sm">
        <div className="flex items-center gap-4">
          <h1 className="text-lg font-bold text-white">DS2工数ダッシュボード</h1>
          {/* View toggle */}
          <div className="flex bg-white/20 rounded-lg p-0.5 text-sm">
            <button
              onClick={() => setViewMode("member")}
              className={`px-3 py-1 rounded-md transition ${
                viewMode === "member" ? "bg-white text-[#1e293b] shadow font-medium" : "text-white/80"
              }`}
            >
              メンバー別
            </button>
            <button
              ...
              className={`px-3 py-1 rounded-md transition ${
                viewMode === "project" ? "bg-white text-[#1e293b] shadow font-medium" : "text-white/80"
              }`}
            >
              プロジェクト別
            </button>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={fetchData}
            disabled={loading}
            className="px-3 py-1 bg-white text-[#1e293b] rounded text-sm font-medium hover:bg-white/90 disabled:opacity-50"
          >
            {loading ? "..." : "更新"}
          </button>
          <span className="text-xs text-white/70">{session.user?.name}</span>
          <button
            onClick={() => signOut()}
