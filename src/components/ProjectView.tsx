      ...
      <table className="w-full border-collapse text-xs" style={{ borderSpacing: 0 }}>
        <thead className="sticky top-0 z-30">
          <tr className="bg-white text-slate-500 border-b border-gray-200">
            <th className="sticky left-0 z-40 bg-white px-2 py-1.5 text-left w-[80px]">顧客</th>
            <th className="sticky left-[80px] z-40 bg-white px-2 py-1.5 text-left min-w-[260px]">プロジェクト</th>
            <th className="bg-white px-2 py-1.5 text-left w-[60px]">PM</th>
            <th className="bg-white px-2 py-1.5 text-center w-[40px]">人数</th>
            {months.map((m) => (
              <th key={m} className="bg-white px-1 py-1.5 text-center min-w-[80px]">{fmtMonth(m)}</th>
            ))}
          </tr>
        </thead>
        <tbody>
